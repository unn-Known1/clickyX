...
## 10.7 Deep-i18n pass (2026-09-22) — the §10.6 residual is closed

Every user-visible literal in `src/` is now a `t()` lookup with EN+ES translations (FR/JA fall back to EN via `fallbackLng`): Home (greetings, dock, empty CTA, starter chips — user-typed recents stay as typed), StatusBar (mic/capture/stats/type-mode/attention, with `{{count}}` interpolation), AgentHUD, Chat (bubbles, sidebar, input, dialogs, toasts), Agents (cards, create form, detail, delete confirm, toasts), all nine settings sections (incl. bridge-token flows, log viewer, PTT presets, provider forms, permission labels, capture controls, appearance variants), onboarding wizard (steps resolve via `onboard.steps.<id>`), About/UpdateBanner/ConfirmDialog, App chrome (tabs, splash, error boundary via `<Translation>`, toasts), palette footer, HotkeyInput, ScreenPreview, VoiceDiscovery, ModelSelector, ModelGeneratorTab, overlay `Calibrating…` (overlay entry now bootstraps the shared i18n module). Deliberately untranslated: brand/provider names, log-level filter values (match backend levels), `Esc`/key-cap labels, cron examples. Test mock resolves keys against real `en.json` **with `{{var}}` interpolation** so tests assert real rendered strings. New exhaustive-deps warnings from `t` in callbacks fixed by declaring `t` in dep arrays (correct: callbacks refresh on language change). Verification: `tsc` clean · vite build pass · **15 files / 113 tests pass** · ESLint src+e2e **0 errors, 28 warnings** · `npm audit` 0 high.
